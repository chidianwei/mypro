...
public class FuncAction extends BaseAction {
	...
	public String sql() {
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				func = list.get(i);
				System.out.println("insert into freecms_func values('"
						+ func.getId() + "','" + func.getName() + "','"
						+ func.getIsok() + "'," + func.getOrdernum() + ",'"
						+ func.getUrl() + "','" + func.getParid() + "');");
			}
		}
		return null;
	}
	...
	public String son() {
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				if (noid != null && noid.trim().length() > 0
						&& noid.equals(list.get(i).getId())) {
					continue;
				}
	...
	public String sonUrl() {
		...
		if ("root".equals(root)) {
			...
			if (isAdminLogin()) {
				...
				list = funcService.selectByParidAuth(root, getLoginAdmin()
						.getId());
			}
		}
		// 生成树
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				if (list.get(i).getIsok().equals("1")) {
					...
					if (list.get(i).getUrl() != null
							&& list.get(i).getUrl().trim().length() > 0) {
			...
			if (list != null && list.size() > 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (role != null && role.getId() != null
						...
						roleFunc = roleFuncService.findRoleFunc(role.getId()
								.trim(), list.get(i).getId());
					}
					stringBuilder
							.append("{ \"text\": \"<input type='checkbox' id='func");
					stringBuilder.append(list.get(i).getId());
		...
		try {
			if (func.getId() != null && func.getId().trim().length() > 0) {
				...
				if (currfunc != null) {
					...
					sb.append("<属性>");
					sb.append(currfunc.getOrdernum() != null ? currfunc
							.getOrdernum() : "");
					sb.append("<属性>");
