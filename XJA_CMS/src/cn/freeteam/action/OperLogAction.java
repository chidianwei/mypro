...
public class OperLogAction extends BaseAction {
	...
	public String delete() throws ParseException {
		...
		if (info != null) {
			if (info.getInfostarttime() != null
					&& info.getInfoendtime() != null) {
				criteria.andOpertimeBetween(info.getInfostarttime(),
						info.getInfoendtime());
			} else {
				if (info.getInfostarttime() != null) {
					criteria.andOpertimeGreaterThanOrEqualTo(info
							.getInfostarttime());
				}
				if (info.getInfoendtime() != null) {
					criteria.andOpertimeLessThanOrEqualTo(info.getInfoendtime());
				}
			}
		}
		if (operlog != null && operlog.getLoginname() != null
				&& operlog.getLoginname().trim().length() > 0) {
			criteria.andLoginnameLike("%" + operlog.getLoginname().trim() + "%");
		}
		int i = operlogsMapper.deleteByExample(example);
		DBCommit();
		return "success";
	}
	...
	public String list() {
		...
		if (operlog != null && operlog.getContent() != null
				&& operlog.getContent().trim().length() > 0) {
			criteria.andContentLike("%" + operlog.getContent().trim() + "%");
		}
		if (operlog != null && operlog.getIp() != null
				&& operlog.getIp().trim().length() > 0) {
