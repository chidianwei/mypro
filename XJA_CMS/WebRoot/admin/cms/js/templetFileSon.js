...
function edit(){
	if(isCheckOne()){
		location.href="templet_fileEdit.do?filePaths="+encodeURI(getCheckOneValue())+"&currFolder="+encodeURI($("#root").val());
	}else{
		alert("请选择一条记录!");
	}
}
...
function del(){
	if(isCheck()){
		if(confirm("确定删除操作么?此操作无法回退!")){
			$.post("templet_fileDel.do","filePaths="+encodeURI(getCheckValue()),delComplete,"text");
		}
	}else{
		alert("请选择要操作的记录!");
	}
