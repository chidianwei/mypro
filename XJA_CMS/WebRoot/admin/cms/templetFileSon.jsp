...
<HTML>
	...
	<BODY style="padding:0;margin:0">
      ...
      <input ${currFolder =='根目录'?'style="display:none"':'' } type="button" value="重命名此文件夹" onclick="renameFolder();"  class="button"  />&nbsp;
      <input ${currFolder =='根目录'?'style="display:none"':'' } type="button" value="删除此文件夹" onclick="delFolder();"  class="button"  />&nbsp;
							...
							for(int i=0;i<sonFiles.size();i++){
								...
								<TR class="tdbg" onMouseOver="this.className='tdbg-dark';"  id="tr<%=URLEncoder.encode(sonFiles.get(i).getPath(),"utf-8") %>"
									...
									<TD  align="left" id="name<s:property value="id"/>">
										<a title="点击查看/下载" onclick="preview('<%=checkParentPath %>/<%=sonFiles.get(i).getPath().replace(realPath,"").replace("\\","/") %>')" href="#"  ><%=sonFiles.get(i).getName() %></a>
									</TD>
								</TR>
								<%
							}
	...
	function preview(urlstr)
	{
		 
		window.open(urlstr,'','width=400,height=185')
		//window.open(urlstr,500,500);
	}
	//%25E4%25BF%25A1%25E6%2581%25AF%25E5%2588%2597%25E8%25A1%25A8.html
